...
varDecl: (LET|CONSTANT) varlist SM;

varlist: variable CM varlist |variable ;

typ: 'Number'|'String'|'Boolean'|'Array'|'JSON';
variable: var (COLON typ)? (ASSIGN (literal|expr))?;

var: ID (LQP NUMLIT (CM NUMLIT)* RQP)*;
...
assignStmt: ID ASSIGN expr SM| ID LQP expr RQP ASSIGN expr SM;
